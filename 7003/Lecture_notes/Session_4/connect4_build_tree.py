board_depth=7; board_columns=['A','B','C','D','E','F','G'];UCB_constant=1.2
...
def make_move(B,player,col):
    ...
    if (win_col+win_row+win_dia)>=1:
        win=1
    return [error,win]
# Initialize tree and state; state is a list of moves
Player1_Tree={0:{'Parent':-1,'Child':[],'State':[],'Type':'D','n':0,'V':0}}
Player2_Tree={0:{'Parent':-1,'Child':[],'State':[],'Type':'S','n':0,'V':0}}
# Build tree
import copy
...
def BuildTree(tree, node, k):
    for game in range(k):
            ...
            if tree[pointer]['Type']=='S':
                ...
                while error_move==1:
                    ...
                    for i in range(1,len(tree[pointer]['Child'])):
                        if tree[tree[pointer]['Child'][i]]['UCB']>tree[tree[pointer]['Child'][select_child]]['UCB']:
                            select_child=i
        ...
        if (game+1)/k*100%10 == 0:
            print(math.floor((game+1)/k*100), end="")
# Now implement the code
BuildTree(Player2_Tree, 0, 50000)
# Export the tree
import pandas as pd
tree_data=pd.DataFrame(data=Player2_Tree)
transposed_tree=tree_data.T
transposed_tree.to_csv("Player2_Tree.csv")
